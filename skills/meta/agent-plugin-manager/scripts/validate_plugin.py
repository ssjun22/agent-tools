...
AGENT_TOOLS_ROOT = Path(__file__).resolve().parents[4]

VALID_DEPENDS_KEYS = {"skills", "agents", "rules", "hooks"}

DEP_PATH_MAP = {
    "skills": lambda name: AGENT_TOOLS_ROOT / "skills" / name,
    "agents": lambda name: AGENT_TOOLS_ROOT / "agents" / f"{name}.md",
    "rules": lambda name: AGENT_TOOLS_ROOT / "rules" / f"{name}.md",
    "hooks": lambda name: AGENT_TOOLS_ROOT / "hooks" / name,
}

DEP_DISPLAY_MAP = {
    "skills": lambda name: f"skills/{name}/",
    "agents": lambda name: f"agents/{name}.md",
    "rules": lambda name: f"rules/{name}.md",
    "hooks": lambda name: f"hooks/{name}",
}
