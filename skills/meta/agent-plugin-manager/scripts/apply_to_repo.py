...
# Resolve agent-tools root: script → scripts/ → agent-plugin-manager/ → meta/ → skills/ → root
AGENT_TOOLS_ROOT = Path(__file__).resolve().parent.parent.parent.parent.parent

# Default source paths for each dependency type
SOURCE_PATHS = {
    "skills": AGENT_TOOLS_ROOT / "skills",
    "agents": AGENT_TOOLS_ROOT / "agents",
    "rules": AGENT_TOOLS_ROOT / "rules",
    "hooks": AGENT_TOOLS_ROOT / "hooks",
}
...
def resolve_dependency(
    ...
    - skills: dep_name is a relative path under skills/ (e.g. "dev/code-reviewer")
    ...
    if dep_type == "skills":
        # Source is a directory: skills/{dep_name}/
