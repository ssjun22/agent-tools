...
# Claude Code target directories mapping: plugin dir -> .claude/ subdir
CLAUDE_CODE_DIR_MAP = {
    "rules": "rules",
    "skills": "skills",
    "agents": "agents",
...
def collect_files_to_apply(plugin_path: Path) -> dict[str, list[Path]]:
    """
    Collect files from plugin subdirectories that map to Claude Code dirs.
    Returns dict: { "rules": [...files], "skills": [...files], "agents": [...files] }
    """
    result = {}
    for source_dir, target_dir in CLAUDE_CODE_DIR_MAP.items():
        source_path = plugin_path / source_dir
...
def apply_files(
    ...
    for source_dir, files in files_map.items():
        ...
        source_base = plugin_path / source_dir
...
def main():
    ...
    # Collect files
    files_map = collect_files_to_apply(plugin_path)

    if not files_map:
        print(f"⚠️  Plugin '{args.plugin}' has no rules/, skills/, or agents/ to apply.")
        print("   Only README.md-only plugins have nothing to apply to a repository.")
        sys.exit(0)
    ...
    print(f"\n🚀 Applying plugin '{args.plugin}' to: {repo_path}{mode}")
