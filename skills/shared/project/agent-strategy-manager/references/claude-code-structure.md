...
## Strategy → Claude Code Mapping

When applying a strategy from `agent-tools/strategies/` to a project:

| Strategy directory | Target in project      |
|--------------------|------------------------|
| `rules/`           | `.claude/rules/`       |
| `skills/`          | `.claude/skills/`      |
| `agents/`          | `.claude/agents/`      |

> **Note:** The strategy's `README.md` is not copied automatically, as it serves as documentation for the strategy itself, not as a project rule.

## Reference

- [Claude Code documentation](https://docs.anthropic.com/en/docs/claude-code)
- Use `scripts/apply_to_repo.py` to automate applying a strategy's files to a project.
