...
| 4. @spec-writer | 생성된 OpenSpec artifact 사용자 확인 필요 |
...
| 4. @spec-writer | (항상 멈춤 — 위 표 참조) | — |
| 5. @designer | — | → 6번 |
| 6. @spec-builder | 테스트 실패, tasks 없음, 설계 모순 | → 7번 |
| 7. @spec-checker | CRITICAL 이슈 발견 | → 9번 |
| 9a. @code-reviewer | CRITICAL/HIGH/MEDIUM 이슈 발견 | → (9b와 합산) |
| 9b. @gemini-prompt-evaluator | CRITICAL/HIGH 이슈 발견 (LLM 프롬프트 변경 시에만) | → (9a와 합산) |
...
두 에이전트를 병렬로 실행하고, 결과를 종합하여 판정한다.

#### 9a. `@code-reviewer` — 코드 품질 검사
- 보안, 코드 품질, 패턴 일관성 검사
- 읽기 전용 — 파일 수정 없음

#### 9b. `@gemini-prompt-evaluator` — ADK 프롬프트 품질 평가
- LLM 프롬프트(ADK `LlmAgent` instruction) 변경이 있을 때만 실행
- Gemini 3 특화 기준 + 범용 프롬프트 기준으로 평가
- 읽기 전용 — 파일 수정 없음
- LLM 프롬프트 변경이 없으면 이 스텝을 건너뛴다
...
└── agents/
    ...
    ├── gemini-prompt-evaluator.md ← @gemini-prompt-evaluator (ADK 프롬프트 평가)
...
| `@gemini-prompt-evaluator` | `/gemini3-prompt-reviewer`, `/prompt-reviewer` |
