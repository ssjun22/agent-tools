...
- base 커밋 해시 (`git-ops.sh preflight`가 브랜치 생성 직후 progress.json `base_commit`에 기록한 값을 메인이 읽어 주입 — 세션 컨텍스트에 의존하지 않음) — 2-B의 `<base>..HEAD` diff 기준
- 출력 산출물 경로:
...
다음 순서를 *그대로* 따른다. 특히 의도 부합 판단(2-D)은 **plan 통과 기준 동작 원문을 미리 보면 confirmation bias가 발생하므로** Blind reading 단계를 먼저 통과해야 한다.

### 1. plan에서 *부분적으로* 정보 추출

`03-plan.md`에서 다음만 먼저 읽는다 (동작 계층·의도 산문은 아직 안 본다):

- `## 통과 기준` 섹션에서는 **`- verify:` 라인만** 취한다 — verify 명령 목록. 동작 계층(체크박스 원문·구체화 행)은 이 시점에 읽지 않는다 — 2-D 1단계(blind reading) 완료 후에만 연다 (가드가 절차 1에서 스스로 뚫리는 자기 오염 방지).
- `## 실행 계획` 섹션 — 태스크 ID 목록과 각 태스크가 명시한 변경 파일, `## 인터페이스 계약` 섹션 — 태스크 간 공유 계약(인터페이스/시그니처/스키마/의존성)
...
`git diff --name-only <base>..HEAD`로 실제 변경 파일 목록 추출 (base는 메인이 주입한 **base 커밋 해시** — 브랜치 생성 직후 HEAD = 사이클 시작 직전 커밋). **base 미주입 시** tasks.json 가장 이른 group 커밋의 부모(`<가장 이른 commit>^`)로 파생하되, 리포트에 '파생 base 사용'을 명시한다.

- 허용 집합 = **plan 명시 파일 + refactor 산출물 `## 변경 파일`의 합집합**. refactor 새 파일은 `(new — 원천)`, 범위 밖 파일은 `(파급 — 원인)` 표기가 있어야 인정.
- **허용 집합에는 있는데 실제로는 없음** → 누락 FAIL
- **실제로는 있는데 허용 집합에 없음** (표기 없는 plan-외 변경) → 이탈 FAIL (`.claude/task-pipeline/` 경로는 제외)
...
plan `## 통과 기준`의 동작 계층(체크박스 원문·구체화 행)을 **아직 열지 말고**, `git diff`와 변경 파일들을 직접 읽어 *"이 코드가 무엇을 하는가"*를 항목별로 요약한다.

> 이 단계를 건너뛰면 plan 키워드가 코드에 등장하는 것만으로 PASS를 줘버리는 confirmation bias가 발생한다.

**2단계 — 체크리스트 분해**

이제 plan `## 통과 기준` 동작 계층의 원문 항목만 분해 대상으로 삼아 *검증 가능한 N개 항목*으로 분해한다 — 구체화 행·담당 표기는 판단 보조, 개요의 설계 산문은 배경 참고이지 판정 항목이 아니다. (원문 항목은 사용자가 게이트 ①에서 lock한 문장 — 판정 기준점을 고정해 라운드 간 분해가 흔들리지 않는다.)

검증 가능성으로 판정 경로가 갈린다:

- *"재시도 시 idempotency 보장"* → 명확. 검증 가능 → 3단계로.
- 동작 계층에 **"사람 확인 필요"로 표기된 항목** → **판정 N/A**. 게이트 ②에서 사용자가 승인하고 plan 위험 절에 verify 사각으로 등재된 설계된 상태이므로, 리포트 '사람 검수 대기' 목록에 집계만 하고 Verdict에 불산입한다 (③ 검수로 인계).
- **표기 없이** 분해 결과 검증 불가로 드러난 항목 → 기존대로 **검증 불가 → FAIL** (엄격 원칙). plan이 사각을 표기하지 않은 결함 신호다.
- 예: *"UX가 부드럽게 동작"* → '사람 확인 필요' 표기 有면 N/A(검수 대기) / 표기 無면 FAIL.
...
**'사람 검수 대기'(2-D N/A)는 FAIL이 아니다** — 위 라우팅 표의 의도 누락에 넣지 않는다. Verdict에 불산입하고 리포트 '사람 검수 대기' 목록으로만 넘겨 ③ 검수에서 사람이 판단한다 (plan이 표기한 verify 사각을 매 라운드 재질문하는 소음 방지).
