...
- 입력 산출물 경로:
  - `.claude/task-pipeline/<ts>/01-clarify.md` — `## 테스트 범위`를 plan과 대조하기 위해 Read (작성 합의 ↔ plan 반영 여부 확인용)
...
**테스트 합의 이행** (clarify ↔ plan 대조): `01-clarify.md`의 `## 테스트 범위`가 `작성: O`인데 plan에 테스트 태스크(group `type: test`)도 없고 verify 명령에 테스트 실행도 없으면 → **FAIL**. 단 이건 산출물 결함이 아니라 *plan 자체가 합의를 빠뜨린 결함*이므로(② 게이트가 놓친 백스톱), 재시도할 영향 태스크가 없다. 따라서 **구조 누락(자동 generator 재시도)으로 분류하지 말고**, 아래 5절 FAIL 유형 중 **이탈에 준해** "plan이 clarify 테스트 합의를 누락"으로 본문에 명시한다 — 메인이 사람에게 `plan 수정 / 면제로 수용 / 종료`를 묻는다(자동 재시도 금지). `작성: X`(면제)면 plan에 면제 사유가 있는지만 확인하고 통과 — 면제 자체는 합의된 결정이므로 FAIL이 아니다.
...
| 유형 | 정의 | 메인 라우팅 |
|---|---|---|
| **구조 누락** (객관) | 2-A 태스크 미완료 / 2-B plan→실제 파일 부재 / 2-C 시그니처·스키마 불일치 | 영향 태스크 → `failed`, **자동 재시도** |
...
| **테스트 합의 누락** | clarify `작성: O`인데 plan에 테스트 태스크·verify 없음 (plan 결함) | tasks.json 변경 안 함 — 메인이 사람에게 `plan 수정 / 면제로 수용 / 종료` 질문 (**자동 재시도 금지** — 재시도할 태스크가 없음) |
...
### 다음 라운드 지시 (FAIL인 경우만)
- 실패 유형: 구조 누락 / verify / 의도 누락(2-D) / 이탈 (해당되는 것 모두)
- 구조 누락: <항목 + 영향 태스크>
- verify: <어느 명령 FAIL + 영향 태스크 또는 "추정 불가">
- 의도 누락(2-D): <plan 인용 항목 + 코드 위치 + 왜 미충족> — 메인이 사람에게 올림
...
- **테스트 품질은 검증하지 않는다 (non-goal)**: 테스트가 *의도한 대상을 제대로 커버하는지*·빈 껍데기(`assert True` 류)인지는 보지 않는다. evaluate가 테스트에 대해 강제하는 것은 *"계획된 테스트가 존재하고(2-A) 실제로 통과하는가(verify 축)"* 까지다. 그 이상은 over-engineering으로 보고 다루지 않는다
- 한국어, 마크다운, 간결
